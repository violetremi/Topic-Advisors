...
async def list_persons(company_id: int, db: AsyncSession = Depends(get_db)):
...
async def create_person(company_id: int, data: PersonCreate, db: AsyncSession = Depends(get_db)):
    person = Person(
    ...
    logger.info(f"新增人员: {person.name} ({person.position}) 企业ID={company_id}")
    return person


@router.get("/{person_id}", response_model=PersonOut)
async def get_person(company_id: int, person_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
...
async def delete_person(company_id: int, person_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(Person).where(Person.id == person_id, Person.company_id == company_id)
    )
    person = result.scalar_one_or_none()
